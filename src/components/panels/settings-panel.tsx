...
const categoryLabels: Record<string, { label: string; icon: string; description: string }> = {
  general: { label: 'General', icon: '⚙', description: 'Core Agent Control Room settings' },
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Configure Agent Control Room behavior and retention policies</p>
        </div>
        ...
        {categories.map(cat => {
          ...
          return (
            ...
            >
              {meta.label}
              {changedCount > 0 && (
                <span className="ml-1.5 inline-flex items-center justify-center w-4 h-4 text-2xs rounded-full bg-violet-500 text-primary-foreground">
                  {changedCount}
                </span>
              )}
            </button>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isChanged && (
                      <span className="text-2xs px-1.5 py-0.5 rounded bg-violet-500/15 text-violet-600 dark:text-violet-400">modified</span>
                    )}
                  </div>
                  ...
                  {isBooleanish ? (
                    <button
                      ...
                      className={`w-10 h-5 rounded-full relative transition-colors ${
                        currentValue === 'true' ? 'bg-violet-500' : 'bg-muted'
                      }`}
                    >
