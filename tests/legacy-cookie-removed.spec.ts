 ...
 * Verifies that the old agent-control-room-auth cookie no longer authenticates.
 */
...
test.describe('Legacy Cookie Auth Removed (Issue #7)', () => {
  test('legacy cookie does not authenticate API requests', async ({ request }) => {
    const res = await request.get('/api/agents', {
      headers: {
        'cookie': 'agent-control-room-auth=test-legacy-secret'
      }
    })
